...
#' @param colour_by The column to colour the plot lines by. Can be an option between the score ("fss"), scale or threshold.
...
plot_fss <- function(
  ...
  # if (length(unique(plot_data$model))>1) {
        # plot_type = "line"
        # plot_data <- plot_data %>% gather(colour_by,"model",c(paste(unique(plot_data$threshold)))) #convert to long table for facet_wrap
  # } # TODO
  ...
  if (plot_type == "line") {
        ...
        # if (length(unique(plot_data$model))>1) {
            # gg <- gg + ggplot2::facet_wrap(. ~ colour_by, ncols = num_facet_cols)
        # } # TODO
  }
