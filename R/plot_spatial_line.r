...
#' @param x_label Label for the x-axis. Set to "auto" to use "Forecast length". Anything else inside
...
plot_spatial_line <- function(
  ...
  ggplot2::scale_x_continuous(breaks = seq(min(plot_data$leadtime), max(plot_data$leadtime,na.rm=TRUE), by = plot_data$leadtime[2]-plot_data$leadtime[1])) +
  ggplot2::labs(title=paste("Score: ",score_lab,", Param: ",unique(plot_data$prm)), y = y_label, x = x_label, colour="Model")
