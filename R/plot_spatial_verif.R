...
plot_spatial_verif <- function(
  ...
  if (!is.object(verif_data) && is.character(verif_data)) {
    if (grepl(".sqlite",verif_data)) {
      message("Input is an .sqlite, not an R object.")
      library(RSQLite)
      message(verif_data)
  ...
  if (any(!is.element(c("fcdate","leadtime"),names(plot_data)))) {
    ...
    ## valid dates, but probably not necessary
    validbdate <- strftime(min(plot_data$fcdates, na.rm=TRUE),format = "%d-%m-%Y %H:%M")
    validedate <- strftime(max(plot_data$fcdates, na.rm=TRUE),format = "%d-%m-%Y %H:%M")
    ## filename dates
  ...
  # apply filtering
  usedModels <- paste(unique(plot_data$model),sep="-") 
  usedParams <- paste(unique(plot_data$prm),sep="-")
  ...
  if (show_data) {
      ...
      message("Model(s): ",usedModels)
      message("Parameter(s): ",usedParams)
  ...
  if (myPlotFunc == "plot_basic") {
    ...
    if (length(ldts)>5) {
      plot_subtitle <- paste("Period:",fcbdate,"-",fcedate,":: Hours {",paste(c(sprintf("%02d", valid_hours)), collapse=", "),"}",":: Leadtimes {",paste(ldts[1:3], collapse=", "),", ...,",ldts[length(ldts)],"}")
    } else {
      plot_subtitle <- paste("Period:",fcbdate,"-",fcedate,":: Hours {",paste(c(sprintf("%02d", valid_hours)), collapse=", "),"}",":: Leadtimes {",paste(ldts, collapse=", "),"}")
    }
  }
  
  if (plot_caption == "auto") {
    plot_caption <- paste("Verification for",usedParams)
  } 
  ...
  # finished
  if (save_image) {
    fname = paste(score_name,"_",savebdate,"_",saveedate,"_",usedModels,"_",usedParams,".png", sep="")
    message("Saving as: ", fname)
    ggplot2::ggsave(filename=fname, dpi=96, width=800, height = 600, units="px")
  } else {
    print(gg)
  }
