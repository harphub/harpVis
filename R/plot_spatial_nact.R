...
#' @param x_label Label for the x-axis. Set to "auto" to use threshold or scale depending on the data. Anything else inside
#'   quotes will be used as the x-axis label.
#' @param flip_axes Logical of whether to swap the x and y axes.
#' @param nact_scores Actual scores to plot. Available scores based on NACT are "fbias","pod","far","pss","hss","sedi".
#' @param colour_by The column to colour the plot lines by. Can be an option between scale or threshold.
...
plot_spatial_nact <- function(
  ...
  if (any(!is.element(c("threshold", "scale", "hit", "fa", "miss", "cr"), names(plot_data)))) stop("plot_data must have columns named threshold, scale, hit, fa, miss and cr!")
  ...
  plot_data <- plot_data %>% dplyr::group_by(model,prm,threshold,scale) %>% dplyr::summarise(hit = mean(hit,na.rm=TRUE),fa = mean(fa,na.rm=TRUE),miss = mean(miss,na.rm=TRUE),cr = mean(cr,na.rm=TRUE))

  if ("fbias" %in% nact_scores) {
      nact_score = "fbias" # Frequency bias
      plot_data[nact_score] <- (plot_data$hit+plot_data$fa)/(plot_data$hit+plot_data$miss)
  }
  if ("pod" %in% nact_scores) {
      nact_score = "pod" # Probability of detection
      plot_data[nact_score] <- plot_data$hit/(plot_data$hit+plot_data$miss)
  }
  if ("far" %in% nact_scores) {
      nact_score = "far" # False-alarm ratio
      plot_data[nact_score] <- plot_data$fa/(plot_data$hit+plot_data$fa)
  }
  if ("pss" %in% nact_scores) {
      nact_score = "pss" # Pierce skill score
      plot_data[nact_score] <- (plot_data$hit/(plot_data$hit+plot_data$fa)-(plot_data$fa/(plot_data$fa+plot_data$cr)))
  }
  if ("hss" %in% nact_scores) {
      nact_score = "hss" # Heidke skill score
      Ar <- ((plot_data$hit+plot_data$fa)*(plot_data$hit+plot_data$miss))/(plot_data$hit+plot_data$fa+plot_data$miss+plot_data$cr) 
      Dr <- ((plot_data$fa+plot_data$cr)*(plot_data$miss+plot_data$cr))/(plot_data$hit+plot_data$fa+plot_data$miss+plot_data$cr) 
      plot_data[nact_score] <- ((plot_data$hit+plot_data$cr-Ar-Dr)/(plot_data$hit+plot_data$fa+plot_data$miss+plot_data$cr-Ar-Dr))
  }
