...
#' @param quantiles A vector of quantiles that will be used to
#'   highlight an area of S/A values within those quantiles.
#' @param xylim Plot x/y axis shared limit. Default "auto" will generate
#'   limits based on plot data.
#' @param show_stats An option to show median and mean values of
#'   S, A and L as a table next to the plot. Default FALSE.
...
plot_spatial_sal <- function(
  ...
  if (any(!is.element(c("S", "A", "L"), names(plot_data)))) {
    stop("plot_data must have columns named S, A and L !")
  }
  ...
  if (is.element("model", names(plot_data)) && length(unique(plot_data$model)) > 1) {
    stop("You are mixing several models in a single SAL plot.")
  }

  if (is.element("prm", names(plot_data)) && length(unique(plot_data$prm)) > 1) {
    stop("You are mixing several parameters in a single SAL plot.")
  }

  if (xylim == "auto") {
    xylim <- max(pmax(plot_data$S,
                      plot_data$A,
                      plot_data$L,
                      na.rm = TRUE),
                 na.rm = TRUE) + 0.25
  ...
  mytable <- cbind(data.frame(stats = c("median S",
                                        "median A",
                                        "median L",
                                        "mean S",
                                        "mean A",
  ...
  gg <- gg +
        ...
        ggplot2::geom_vline(xintercept = 0,
                            colour = "grey80") +
        ggplot2::geom_hline(aes(yintercept = medianA,
                                linetype   = "median"),
                            colour = "grey80") +
        ggplot2::geom_vline(aes(xintercept = medianS),
                            linetype = "dashed",
                            colour = "grey80") +
